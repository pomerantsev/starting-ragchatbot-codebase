...
class TestRealSystemDiagnosis:
    ...
    def test_vector_store_connection(self):
        ...
        try:
            ...
            assert course_count >= 0, "Vector store should return non-negative course count"
            
            if course_count == 0:
                pytest.fail("No courses found in vector store - this explains 'query failed'")
    ...
    def test_vector_store_search_functionality(self):
        ...
        try:
            ...
            print(f"Search results for 'Python': {len(results.documents) if not results.error else 'ERROR: ' + results.error}")
            ...
            if course_titles:
                ...
                results = self.vector_store.search("introduction", course_name=first_course)
                print(f"Search results for 'introduction' in '{first_course}': {len(results.documents) if not results.error else 'ERROR: ' + results.error}")
                
                if results.error:
                    pytest.fail(f"Vector store search with course filter failed: {results.error}")
    ...
    def test_course_search_tool_with_real_data(self):
        ...
        try:
            ...
            print(f"CourseSearchTool result for 'Python programming': {result[:200]}...")
            
            # Check for error conditions
            if "No relevant content found" in result:
                course_count = self.vector_store.get_course_count()
                if course_count == 0:
                    pytest.fail("CourseSearchTool returns 'No relevant content found' because vector store is empty")
                else:
                    print(f"CourseSearchTool found no content despite {course_count} courses in vector store")
            ...
            if course_titles:
                ...
                print(f"CourseSearchTool result with course filter '{first_course}': {result[:200]}...")
    ...
    def test_chroma_db_persistence(self):
        ...
        try:
            ...
            if not db_files:
                pytest.fail("ChromaDB directory exists but is empty - no data persisted")
    ...
    def test_course_name_resolution(self):
        ...
        try:
            ...
            assert resolved == first_course, f"Exact match should return same course name"
            ...
            if len(first_course.split()) > 1:
                ...
                assert resolved is not None, f"Partial match should resolve to some course"
    ...
    def test_tool_manager_functionality(self):
        ...
        try:
            ...
            # Test tool execution
            search_result = self.tool_manager.execute_tool("search_course_content", query="Python")
    ...
    def test_ai_generator_initialization(self):
        ...
        try:
            ...
            assert hasattr(ai_gen, 'SYSTEM_PROMPT'), "AIGenerator should have SYSTEM_PROMPT"
    ...
    def test_docs_folder_existence(self):
        ...
        try:
            ...
            doc_files = [f for f in os.listdir(docs_path) if f.lower().endswith(('.pdf', '.txt', '.docx'))]
            print(f"Document files found: {doc_files}")
            
            if not doc_files:
                pytest.fail("Docs folder exists but contains no document files - this explains why vector store is empty")
